  ...
  */
  ...
  totalContactForce = 0.0;
	...
	for (int i = 0; i < rod->nv; i++)
	{
      ...
      /*
      // start friction
      Vector3d x0_old = rod->getVertexOld(i);
      Vector3d u0_old = rod->getVelocityOld(i);

      Vector3d forceOld = computeContactForce(x0_old(0), x0_old(1), x_0(2), x_1(0), x_1(1), x_1(2), 
        x_2(0), x_2(1), x_2(2), radius, dbar);

      forceOld = - forceOld * stiffness;

      Vector3d tubeTangent = (x_2 - x_1) / (x_2 - x_1).norm(); 
      Vector3d edgeVec = (x0_old - x_1) / (x0_old - x_1).norm(); 

      Vector3d normDirection = tubeTangent.cross(edgeVec.cross(tubeTangent));

      Vector3d xNorm = normDirection / normDirection.norm();
      Vector3d v_rt = u0_old - u0_old.dot(xNorm) * xNorm;
      Vector3d tK;
      ...
      else
      {
        ...
        tK = v_rt / (v_rt.norm() + 1e-8);
      }

      Vector3d friction = - mu * forceOld.norm() * fVelocity * tK;

      for (int ii = 0; ii < 3; ii++)
      {
          int ind = 4 * i + ii;
          stepper->addForce(ind, - friction(ii));
      }
      // end friction
      */
      ...
      if (i == rod->nv - 1)
      {
        //Vector3d totalheadForce = forceOld + friction;

        //cout << "norm: " << forceOld.transpose() << endl;
        //cout << "tang: " << friction.transpose() << endl;
